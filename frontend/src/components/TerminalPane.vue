...
import { api, shellUrl, terminalUrl } from '../api.js'
const props = defineProps({ session: Object, kind: { type: String, default: 'agent' }, active: { type: Boolean, default: true }, readonly: { type: Boolean, default: false } }); const emit = defineEmits(['status'])
...
async function connect() { const socket = new WebSocket(await (props.kind === 'shell' ? shellUrl(props.session.id) : terminalUrl(props.session.id))); ws = socket; socket.onopen = () => { emit('status', 'connected'); resize() }; socket.onmessage = event => term.write(typeof event.data === 'string' ? event.data : new Uint8Array(event.data)); socket.onclose = () => { emit('status', 'disconnected'); if (isLive.value) setTimeout(() => term && connect(), 2000) }; socket.onerror = () => emit('status', 'error') }
async function transcript() { emit('status', 'history'); term.write((await api.getTranscript(props.session.id)) || '\r\n[no saved transcript]\r\n') }
