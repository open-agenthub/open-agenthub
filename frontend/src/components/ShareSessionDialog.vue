...
async function load() {
  try { data.value = await api.listSessionShares(props.session.id); blockedServers.value = (data.value.policy?.blockedServers || []).join('\n'); blockedTools.value = (data.value.policy?.blockedTools || []).join('\n') }
...
<template>
  <div :class="embedded ? 'embed' : 'overlay'" @click.self="embedded || $emit('close')"><div :class="embedded ? 'embed-inner' : 'modal'"><h3>Share session</h3><p class="note">Roles control terminal input. Session settings, shell access, projects, and sharing remain owner-only.</p>
    ...
    <section><h4>Secret links</h4><div class="inline"><select v-model="role"><option>Viewer</option><option>Collaborator</option></select><input v-model="expiresAt" type="datetime-local" /><button @click="createLink">Create link</button></div><p v-if="oneTimeUrl" class="one-time">Copy this one-time link now; it cannot be shown again.<button @click="copyLink">Copy</button><code>{{ oneTimeUrl }}</code></p><div v-for="link in links" :key="link.id" class="item"><span>{{ link.id }} · {{ link.role }}<small v-if="link.expiresAt"> · expires {{ link.expiresAt }}</small></span><button class="danger" @click="removeLink(link)">Revoke</button></div></section>
