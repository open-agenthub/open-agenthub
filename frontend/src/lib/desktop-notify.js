...
export function detectAlerts(prev, next) {
  ...
  for (const s of next) {
    ...
    else if (old.phase !== s.phase && TERMINAL[s.phase]) alerts.push({ id: s.id, title: s.title, kind: TERMINAL[s.phase] })
  }
  return alerts
}
...
export function showAlert(alert, onOpen) {
  ...
  const n = new Notification('Open AgentHub', { body: `Session ${alert.title} ${body}.`, tag: `agenthub-${alert.id}` })
