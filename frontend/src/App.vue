...
onMounted(async () => { if (needsLogin) return; await refresh(); setInterval(refresh, 5000); try { isAdmin.value = (await api.adminAccess()).isAdmin } catch {} })
</script>
<template>
  <div v-if="needsLogin" class="login"><div class="login-card"><h1>Open AgentHub</h1><p>Please sign in to manage your agent sessions.</p><button class="primary" @click="auth.login()">Sign in</button></div></div>
